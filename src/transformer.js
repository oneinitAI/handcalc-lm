...
<section class="card">
  ...
  <p class="muted">你的手算 LM 就是一个微型 Transformer：几百个参数，但注意力、多头、位置编码全都真材实料。这一页把它的每一颗螺丝拆给你看，<b>全程不说一个不解释的黑话</b>。</p>
</section>
...
<section class="card">
  ...
  <table class="ms-table" style="width:100%">
    ...
    <tr><td><b>贵不贵</b></td><td>O(n×窗口)，便宜</td><td>O(n²)，句子越长越贵（每个词看每个词）</td></tr>
  </table>
  <div class="howto">注意最后一行：注意力算 n² 次是它的软肋——这也是后来 Flash Attention、稀疏注意力等加速技术都在攻克的点（「前沿」tab 有讲）。</div>
</section>
...
<section class="card">
  ...
  <div class="teach">
    <h3>零件 ① 词嵌入＝查字典翻译官</h3>
    <p><b>问题</b>：模型只会算数字，不会看汉字。怎么办？准备一本<b><a class="gloss-link" data-gloss="Embedding">大字典（嵌入表）</a></b>：每个字对应一串数字（叫<b>向量</b>，你可以理解成"这个字的身份证号"，只不过一位多长、有方向有大小）。查表这一步，就是在给每个 <a class="gloss-link" data-gloss="Token">token</a>（模型眼里的一个字）编号。</p>
    <p class="mono">我 → [0.12, -0.34, 0.55]　爱 → [-0.21, 0.68, 0.10]　你 → [0.43, 0.05, -0.72]</p>
    <p><b>训练时这本字典会一直改</b>：意思相近的字，身份证号也越来越像。去「词典」tab 或文本模型的<b>语义空间</b>，你能亲眼看到"月光"和"荷塘"挤在一起。</p>
  </div>
  ...
  <div class="teach">
    ...
    <p class="howto"><b>公式翻译</b>：pos 是第几个位置，i 是身份证号的第几格，d 是身份证总共几格。每个位置拿到一串"波形指纹"——<b>挨得近的位置指纹像，离得远的不像</b>，模型就分得清先后了。</p>
    <div class="viz">
      <div class="viz-title">座位号长什么样（0→50 号位置，每个维度一条波）</div>
      <canvas id="posEncCanvas" class="canvas"></canvas>
      <p class="muted">同一个维度上，位置越近数值越接近——模型能读出"第 3 个和第 4 个挨着"。</p>
    </div>
  </div>

  <div class="teach">
    <h3>零件 ③ Q K V＝相亲的三样东西</h3>
    <p>每个词进会场前，都把自己的信息复制成三份（各乘一个不同的矩阵），各自派上用场：</p>
    <div class="feat-grid">
...
<section class="card">
  <div class="teach">
    ...
    <p>最后把所有头的结果拼起来，再过一层线性变换——<b>一次会议，多个视角同时看</b>。</p>
  ...
  <div class="teach">
    ...
    <p class="howto">公式翻译：<span class="mono">FFN(x) = 第二层( 激活( 第一层(x) ) )</span>——就是"先加工再加工"的两步处理。注意力和 FFN 交替叠起来，就是完整的一层。</p>
  </div>
  ...
  <div class="teach">
    ...
    <div class="feat-grid">
      <div class="feat"><b>残差连接（传送带）</b><span>把原始输入原样带到后面"加"上——信息不丢，梯度好传（理解成：重要内容永远有直达通道）。</span></div>
...
<section class="card">
  ...
  <div class="feat-grid">
    ...
    <div class="feat"><b>语义空间</b><span>看训练后词向量自动聚类——字典在长什么样。</span></div>
  </div>
...
function drawPosEncoding() {
  ...
  dims.forEach(([i, freq], di) => {
    ...
    ctx.fillText('维度 ' + i + '（波长' + (i === 0 ? '长' : i === 2 ? '中' : '短') + '）', 6, 14 + di * 16)
  })
}
